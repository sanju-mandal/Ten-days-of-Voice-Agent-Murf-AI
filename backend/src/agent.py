    ...
    try:
        ...
        if cur.fetchone()[0] == 0:
            catalog = [
                ...
                ("cake-choco-1kg", "Chocolate Birthday Cake", "Birthday Party", 550.00, "Bakers Delight", "1kg", "box", json.dumps(["cake", "party", "sweet"])),
                ("candles-pack", "Birthday Candles Pack", "Birthday Party", 30.00, "PartyPop", "1 pack", "pack", json.dumps(["candles", "birthday"])),
                ("banner-happybday", "Happy Birthday Banner", "Birthday Party", 80.00, "PartyPop", "1 unit", "pack", json.dumps(["decor", "banner"])),
                ("caps-party-10", "Party Caps (10 pcs)", "Birthday Party", 60.00, "PartyPop", "10 pcs", "pack", json.dumps(["decor", "kids"])),
                ("balloons-50", "Colorful Balloons (50 pcs)", "Birthday Party", 120.00, "PartyPop", "50 pcs", "pack", json.dumps(["decor", "balloons"])),
                ("return-gifts-10", "Kids Return Gift Set (10 pcs)", "Birthday Party", 250.00, "FunBox", "10 pcs", "pack", json.dumps(["gifts", "kids"])),
                ("knife-cake", "Cake Cutting Knife", "Birthday Party", 20.00, "Bakers Delight", "1 unit", "piece", json.dumps(["cake", "party"])),
                ("party-spray", "Party Celebration Spray", "Birthday Party", 95.00, "FunBlast", "1 unit", "can", json.dumps(["party", "celebration"]))
...
class FoodAgent(Agent):
    def __init__(self):
        super().__init__(
            instructions="""
            You are 'Shaan', a helpful assistant for 'Mr. Sunju's Shop', an Indian grocery store.
            Currency is Indian Rupees (₹).
            
            Capabilities:
            1. Catalog: Search for Indian items (Amul milk, Tata salt, Maggi, Basmati rice, etc).
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info("\n" + "🇮🇳" * 12)
    logger.info("🚀 STARTING MR. SUNJU'S SHOP (Indian Context + Auto-Tracking)")
